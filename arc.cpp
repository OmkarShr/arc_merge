...
// Perp distance from point c to line ab
...
// Returns a subset of points that approximates the polyline within "tolerance"
static std::vector<Point> rdpSimplify(const std::vector<Point> &pts, double tolerance) {
    if (pts.size() < 3) {
        return pts; // No simplification needed
    }
    ...
    // If max distance is within tolerance, remove all interior points
    if (maxDist < tolerance) {
        ...
        // Merge but avoid duplicating the midpoint
        leftSimpl.pop_back();
...
std::vector<std::complex<double>> dft(const std::vector<double>& data) {
    ...
    for(size_t k = 0; k < n; k++) {
        ...
        for(size_t t = 0; t < n; t++) {
            double angle = 2.0 * PI * (double)k * (double)t / (double)n;
            sum += std::complex<double>(data[t] * cos(angle), -data[t] * sin(angle));
        }
        result[k] = sum;
    }
    return result;
    ...
    for(size_t t = 0; t < n; t++) {
        ...
        for(size_t k = 0; k < n; k++) {
            ...
            sum += std::complex<double>(
               freqData[k].real() * cos(angle) - freqData[k].imag() * sin(angle),
               freqData[k].real() * sin(angle) + freqData[k].imag() * cos(angle)
            );
        }
...
// Apply a simple FFT-like filter by zeroing out high-frequency amplitudes
void applyFFTFilter(std::vector<Point>& points, double /*toler*/) {
    ...
    // Example approach: keep low 20% of frequencies
    size_t cutoff = std::min((size_t)(xFreq.size() * 0.2), xFreq.size());
...
// Convert simplified points into line/arc segments
std::vector<Segment> buildSimplifiedSegments(const std::vector<Point>& points, double toler) {
    ...
    // handle leftover pair or single
    if(i + 1 < points.size()) {
        Segment seg;
        seg.start = points[i];
        seg.end = points.back();
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        // 3) Optional: apply naive FFT filter
        applyFFTFilter(points, tolerance);

        // 4) RDP simplification => fewer points for bigger tolerance
        points = rdpSimplify(points, tolerance);

        // 5) Build final segments from simplified points using curvature
